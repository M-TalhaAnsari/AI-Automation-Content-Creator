...
def disable_rate_limiting():
    """This file tests session/history wiring, not rate limiting
    (that's web/test_rate_limit.py's job) -- several tests here make
    3+ /chat calls each, all sharing identities like 'testclient' or
    the real 'web-real-key', and would otherwise risk tripping the
    real 10/minute limit partway through an unrelated test run."""
...
def fake_redis(monkeypatch):
    ...
    # Fake the job queue's connection too -- without this, any test that
    # triggers a slow action (run_new_request etc.) pushes a real job
    # into the REAL Redis queue, which then gets picked up and actually
    # executed later by a real worker. Confirmed happening in practice.
