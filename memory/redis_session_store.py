...
Key shape:  tf:session:{session_id}  ->  JSON string of the conversation dict
...
def load_conversation(session_id: str) -> Dict[str, Any]:
    """
    Returns the conversation dict for this session_id, or a fresh default
    dict if none exists yet, the entry is corrupted, or Redis is
    unreachable.
    ...
    try:
        ...
        logger.warning("redis_session_store.load_conversation: Redis unreachable for %s: %s", session_id, e)
    ...
    try:
        ...
        logger.warning("redis_session_store.load_conversation: corrupted entry for %s: %s", session_id, e)
        return _fresh_default()
...
def save_conversation(session_id: str, conversation: Dict[str, Any]) -> bool:
    """
    Serializes and saves conversation, refreshing its TTL. Returns True on
    success, False on failure. Never raises -- a failed save is logged,
    not fatal to a request that already computed a valid response.
    """
    try:
        ...
        logger.error("redis_session_store.save_conversation: non-JSON-safe conversation for %s: %s", session_id, e)
        return False
    ...
    try:
        ...
        logger.warning("redis_session_store.save_conversation: Redis unreachable for %s: %s", session_id, e)
        return False
...
def delete_conversation(session_id: str) -> bool:
    ...
    try:
        ...
        logger.warning("redis_session_store.delete_conversation: Redis unreachable for %s: %s", session_id, e)
        return False
